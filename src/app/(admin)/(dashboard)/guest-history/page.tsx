...
import reservations from "@/components/admin/guest-history/dataHistory";
					...
					<thead>
						<tr>
							{[
								...
								<th
									key={idx}
									className="text-[#5D6679] font-semibold text-sm"
								>
									{header}
								</th>
							))}
					...
					<tbody>
						{getPaginatedData().length > 0 ? (
							getPaginatedData().map((reservation) => (
								<tr key={reservation.id} className="text-gray-400 text-sm border-b">
									<td className="py-4">
										{reservation.id}
									</td>
									<td className="py-4">
